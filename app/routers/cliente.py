...
@router.post("/", response_model=ClienteCreate, tags=["Clientes"])
...
async def listar_clientes(db: AsyncSession = Depends(get_async_session)):
    return await crud_cliente.create_cliente(db)

@router.get("/{cliente_id}", response_model=Cliente, tags=["Clientes"])
